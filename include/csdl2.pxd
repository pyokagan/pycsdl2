...
cdef extern from 'SDL_stdinc.h' nogil:
    ...
    Uint32 SDL_FOURCC(Uint8 A, Uint8 B, Uint8 C, Uint8 D)
    ...
    size_t SDL_strlcpy(char *str, const char *src, size_t maxlen)
    ...
    Uint16 *SDL_iconv_utf8_ucs2(const char *inbuf)

    Uint32 *SDL_iconv_utf8_ucs4(const char *inbuf)


cdef extern from 'SDL_assert.h' nogil:
    enum:
        SDL_ASSERT_LEVEL
        SDL_FUNCTION
        SDL_FILE
    ...
    void SDL_assert(int condition)

    void SDL_assert_release(int condition)

    void SDL_assert_paranoid(int condition)

    ctypedef SDL_assert_state (*SDL_AssertionHandler)(const SDL_assert_data *data,
                                                      void *userdata)
...
cdef extern from 'SDL_atomic.h' nogil:
    ...
    int SDL_AtomicSet(SDL_atomic_t *a, int oldval, int newval)
...
cdef extern from 'SDL_audio.h' nogil:
    ...
    SDL_AudioFormat SDL_AUDIO_ISINT(SDL_AudioFormat x)

    SDL_AudioFormat SDL_AUDIO_ISLITTLEENDIAN(SDL_AudioFormat x)

    SDL_AudioFormat SDL_AUDIO_ISUNSIGNED(SDL_AudioFormat x)
    ...
    int SDL_BuiltAudioCVT(SDL_AudioCVT *cvt, SDL_AudioFormat src_format,
                          Uint8 num_channels, int src_rate,
...
cdef extern from 'SDL_bits.h' nogil:
...
cdef extern from 'SDL_clipboard.h' nogil:
...
cdef extern from 'SDL_cpuinfo.h' nogil:
    enum:
        SDL_CACHELINE_SIZE
...
cdef extern from 'SDL_endian.h' nogil:
    enum:
        SDL_LIL_ENDIAN = 1234
        SDL_BIG_ENDIAN = 4321
...
cdef extern from 'SDL_joystick.h' nogil:
    ...
    void SDL_JoystickGetGUIDString(SDL_JoystickGUID guid, char *pszGUID,
                                   int cbGUID)
...
cdef extern from 'SDL_gamecontroller.h' nogil:
    ...
    struct _SDL_GameControllerButtonBind_hat:
        int hat
        int hat_mask

    union _SDL_GameControllerButtonBind_value:
        int button
        int axis
        _SDL_GameControllerButtonBind_hat hat

    ctypedef struct SDL_GameControllerButtonBind:
        SDL_GameControllerBindType bindType
        _SDL_GameControllerButtonBind_value value
...
cdef extern from 'SDL_haptic.h' nogil:
...
cdef extern from 'SDL_hints.h' nogil:
    ...
    ctypedef void (*SDL_HintCallback)(void *userdata, const char *name,
                                      const char *oldValue,
                                      const char *newValue)
...
cdef extern from 'SDL_loadso.h' nogil:
...
cdef extern from 'SDL_log.h' nogil:
    enum:
    ...
    ctypedef void (*SDL_LogOutputFunction)(void *userdata, int category,
...
cdef extern from 'SDL_messagebox.h' nogil:
    ...
    ctypedef enum SDL_MessageBoxButtonFlags:
        SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT = 0x01
        SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT = 0x02
    ...
    # NOTE: colors[] array size needs to be modified if number of enum
    # elements in SDL_MessageBoxColorType changes.
    ...
    ctypedef int SDL_ShowMessageBox(const SDL_MessageBoxData *messageboxdata,
                                    int *buttonid)

    ctypedef int SDL_ShowSimpleMessageBox(Uint32 flags, const char *title,
                                          const char *message,
                                          SDL_Window *window)
...
cdef extern from 'SDL_mouse.h' nogil:
    ...
    int SDL_SetRelativeMouseMoude(SDL_bool enabled)
...
cdef extern from 'SDL_mutex.h' nogil:
    enum:
        SDL_MUTEX_TIMEDOUT
        SDL_MUTEX_MAXWAIT
...
cdef extern from 'SDL_pixels.h' nogil:
    ...
    Uint32 SDL_DEFINE_PIXELFOURCC(Uint8 A, Uint8 B, Uint8 C, Uint8 D)

    Uint32 SDL_DEFINE_PIXELFORMAT(Uint8 type, Uint8 order, Uint8 layout,
                                  Uint8 bits, Uint8 bytes)

    Uint8 SDL_PIXELFLAG(Uint32 x)

    Uint8 SDL_PIXELTYPE(Uint32 x)

    Uint8 SDL_PIXELORDER(Uint32 x)

    Uint8 SDL_PIXELLAYOUT(Uint32 x)

    Uint8 SDL_BITSPERPIXEL(Uint32 x)

    Uint8 SDL_BYTESPERPIXEL(Uint32 x)

    int SDL_ISPIXELFORMAT_INDEXED(Uint32 format)

    int SDL_ISPIXELFORMAT_ALPHA(Uint32 format)

    int SDL_ISPIXELFORMAT_FOURCC(Uint32 format)
    ...
    Uint32 SDL_MapRGBA(const SDL_PixelFormat *format, Uint8 r, Uint8 g,
                       Uint8 b, Uint8 a)
...
cdef extern from 'SDL_power.h' nogil:
...
cdef extern from 'SDL_quit.h' nogil:
...
cdef extern from 'SDL_render.h' nogil:
    ...
    int SDL_RenderDrawPoints(SDL_Renderer *renderer, const SDL_Point *points,
                             int count)

    int SDL_RenderDrawLine(SDL_Renderer *renderer, int x1, int y1, int x2,
                           int y2)
    ...
    int SDL_RenderDrawRects(SDL_Renderer *renderer, const SDL_Rect *rects,
                            int count)
    ...
    int SDL_GL_BindTexture(SDL_Texture *texture, float *txw, float *texh)
...
cdef extern from 'SDL_rwops.h' nogil:
    ...
    ctypedef struct SDL_RWops:
        ...
        size_t (*read)(SDL_RWops *context, void *ptr, size_t size,
                       size_t maxnum)
        size_t (*write)(SDL_RWops *context, const void *ptr, size_t size,
                        size_t num)
    ...
    size_t SDL_RWwrite(SDL_RWops *ctx, const void *ptr, size_t size,
                       size_t num)
...
cdef extern from 'SDL_shape.h' nogil:
    enum:
        SDL_NOSHAPEABLE_WINDOW
    ...
    int SDL_SHAPEMODEALPHA(WindowShapeMode mode)
    ...
    int SDL_SetWindowShape(SDL_Window *window, SDL_Surface *surface,
...
cdef extern from 'SDL_surface.h' nogil:
    ...
    int SDL_GetSurfaceColorMod(SDL_Surface *surface, Uint8 *r, Uint8 *g,
                               Uint8 *b)
...
cdef extern from 'SDL_video.h' nogil:
    ...
    int SDL_GetWindowGammaRamp(SDL_Window *window, Uint16 *red, Uint16 *green,
                               Uint16 *blue)
...
cdef extern from 'SDL_system.h' nogil:
...
cdef extern from 'SDL_syswm.h' nogil:
    ctypedef enum SDL_SYSWM_TYPE:
        SDL_SYSWM_UNKNOWN
        SDL_SYSWM_WINDOWS
        SDL_SYSWM_X11
        SDL_SYSWM_DIRECTFB
...
cdef extern from 'SDL_thread.h' nogil:
    ...
    ctypedef SDL_Thread *SDL_CreateThread(SDL_ThreadFunction fn,
...
cdef extern from 'pycsdl2.h':
    ctypedef struct PyCSDL2_CAPI

    const PyCSDL2_CAPI *PyCSDL2_Import() except NULL

    # src/audio.h

    object PyCSDL2_AudioSpecCreate(const SDL_AudioSpec *spec)

    int PyCSDL2_AudioSpecPtr(object obj, SDL_AudioSpec **out) except 0

    object PyCSDL2_AudioDeviceCreate(SDL_AudioDeviceID id)

    int PyCSDL2_AudioDeviceID(object obj, SDL_AudioDeviceID *out) except 0

    # src/events.h

    object PyCSDL2_MouseMotionEventCreate(const SDL_MouseMotionEvent *motion)

    int PyCSDL2_MouseMotionEventPtr(object, SDL_MouseMotionEvent **) except 0

    object PyCSDL2_EventCreate(const SDL_Event *ev)

    int PyCSDL2_EventPtr(object obj, SDL_Event **out) except 0

    # src/pixels.h

    object PyCSDL2_PaletteCreate(SDL_Palette *plt)

    int PyCSDL2_PalettePtr(object obj, SDL_Palette **out) except 0

    object PyCSDL2_PixelFormatCreate(SDL_PixelFormat *pfmt)

    int PyCSDL2_PixelFormatPtr(object obj, SDL_PixelFormat **out) except 0

    # src/rect.h

    object PyCSDL2_PointCreate(const SDL_Point *point)

    int PyCSDL2_PointPtr(object obj, SDL_Point **out) except 0

    object PyCSDL2_RectCreate(const SDL_Rect *rect)

    int PyCSDL2_RectPtr(object obj, SDL_Rect **out) except 0

    # src/render.h

    object PyCSDL2_RendererCreate(SDL_Renderer *rdr, object deftarget)

    int PyCSDL2_RendererPtr(object obj, SDL_Renderer **out) except 0

    object PyCSDL2_TextureCreate(SDL_Texture *tex, object rdr)

    int PyCSDL2_TexturePtr(object obj, SDL_Texture **out) except 0

    # src/rwops.h

    object PyCSDL2_RWopsCreate(SDL_RWops *rwops)

    int PyCSDL2_RWopsPtr(object obj, SDL_RWops **out) except 0

    # src/surface.h

    object PyCSDL2_SurfaceCreate(SDL_Surface *sf, object pixels)

    int PyCSDL2_SurfacePtr(object obj, SDL_Surface **out) except 0

    # src/video.h

    object PyCSDL2_WindowCreate(SDL_Window *window)

    int PyCSDL2_WindowPtr(object obj, SDL_Window **out) except 0
